...
func add_item(item):
	...
	var inventory_full = Global.inventory.size() >= inventory_size
	if inventory_full:
...
func append_inventory_item(id, item_name, texture, count):
	...
	Global.inventory.append(inventory_item)
